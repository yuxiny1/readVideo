...
def summarize_transcript_with_client(
    transcript_text: str,
    ...
    if len(chunks) == 1:
        ...
        if not items:
            raise RuntimeError(f"{engine_label}没有返回可用的总结要点。")
...
def build_article_note_with_client(
    transcript_text: str,
    client: TextGenerationClient,
    engine_label: str,
    ...
    if not chunk_notes:
        raise RuntimeError(f"{engine_label}没有返回可用的分段笔记。")
...
def _generate_summary(client: TextGenerationClient, prompt: str) -> list[str]:
