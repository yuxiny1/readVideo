...
- `backend/services/`：下载、Whisper、Ollama/MLX、Markdown 和队列等基础服务。`mlx_whisper_transcription.py` 负责隔离环境中的 Apple GPU 转录，`mlx_whisper_runner.py` 负责解码和相邻片段去重，`mlx_whisper_models.py` 负责 Hugging Face 模型目录、缓存检查和下载。
