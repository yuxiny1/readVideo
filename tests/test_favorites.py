...
class FavoriteStoreTest(unittest.TestCase):
    ...
    def test_create_folder_and_assign_favorite(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            store = FavoriteStore(str(Path(tmpdir) / "favorites.sqlite3"))
            favorite = store.add_from_history(
                HistoryRecord(
            ...
            folder = store.add_folder("AI Notes", "research")
            assigned = store.assign_folder(favorite.id, folder.id)

            self.assertEqual(assigned.folder_id, folder.id)
            self.assertEqual(assigned.folder_name, "AI Notes")
            self.assertEqual(store.list_folders()[0].notes, "research")
