...
from backend.services.task_queue import enqueue_video_processing, run_video_job, run_worker_probe


class TaskQueueTest(unittest.TestCase):
    def test_local_mode_uses_fastapi_background_tasks(self):
        local_scheduler = MagicMock()
        processor = MagicMock()

        backend = enqueue_video_processing(local_scheduler, processor, Settings(), "task-1", "url")

        self.assertEqual(backend, "local")
        local_scheduler.assert_called_once_with(processor, "task-1", "url")
    ...
    def test_redis_mode_enqueues_worker_job(self):
        ...
        local_scheduler.assert_not_called()
        queue.enqueue.assert_called_once()
        self.assertEqual(queue.enqueue.call_args.kwargs["job_id"], "task-1")

    def test_worker_runs_async_video_processor(self):
        processor = AsyncMock()
        with patch("backend.application.handlers.tasks.process_video", processor):
            run_video_job("task-1", "url")
        processor.assert_awaited_once_with("task-1", "url")

    def test_worker_probe_is_dispatched_through_mediator(self):
        self.assertEqual(
            run_worker_probe("probe-1"),
            {"worker": "ready", "mediator": "ready", "value": "probe-1"},
        )
